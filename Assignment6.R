...
# wir validieren diese erste Einschätzungen mit einem qq plot
...
#0.9020131 
# H0 wird akzeptiert, da p>α
# Auf dieser Grundlage können wir nun den t-test durchführen. Wir entscheiden uns
# für einen gepaarten T-Test, weil di Stichproben abhängig sind.
# H0: Die Mittelwerte der Stichproben weichen nicht voneinander ab.
# H1: Die Mittelwerte der Stichproben weichen voneinander ab.

t.test(A, B, alternative = "two.sided", conf.level = 0.95, paired=TRUE)

#Paired t-test
#data:  A and B
#t = -0.74849, df = 99, p-value = 0.4559
#alternative hypothesis: true mean difference is not equal to 0
#95 percent confidence interval:
#  -0.06733244  0.03044767
#sample estimates:
#  mean difference 
#-0.01844239
...
for (i in 1:12) {
  ...
  if (length(subset_data) > 1) {  
    result <- t.test(GB[Sternzeichen == i], subset_data, paired=TRUE)
...
# H0 wird nur für Sternzeichen 4 und 9 akzeptiert. Das heißt es gibt nur für dieses Sternzeichen eine signifikante Abweichung vom Mittelwert
# der Kontrollgruppe. Als Marketer würde ich empfehlen, das Produkt nicht speziell an Jungfrauen zu vermarkten, da ansonsten vielleicht
